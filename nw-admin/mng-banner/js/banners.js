	var table = $('#tblbanners').DataTable( {
		ajax: "control/banners.lista.php",
		responsive: true,
		language:  {
    ...
    Swal.fire({
      ...
      text: "El banner se eliminará completamente",
      type: 'warning',
	...
	$(document).on('click','.pre_edit-sm', function() {
		...
		var movi = $(this).data('movi');
		$('#idban').val(id);
		$('#preveddes img').attr('src','../../img/slider/'+desk);
		$('#prevedmov img').attr('src','../../img/slider/'+movi);
		$('#EdBan').modal('show');
	});
